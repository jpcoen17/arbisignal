...
export const FREE_THRESHOLD = 0.01;     // 0.01% for free users
export const PREMIUM_THRESHOLD = 0.001; // 0.001% for premium users
...
export function findArbitrageOpportunities(
  ...
  const pairs = Array.from(new Set(prices.map((p) => p.pair))) as TradingPair[];
